...
class DenoisingModel(pl.LightningModule):
    ...
    It defines methods for denoising recordings of arbitrary lengths, as well as
    supplying its temporal window length and padding expectations for use in training and inference.
    '''
    def __init__(
            self,
            name: str,
    ...
    def denoise_movie(
        ...
        '''
        Denoises the 'diff' movie segment in ws_denoising, bounded by [t_begin, t_end)
        and windowed by (x0, y0, x_window, y_window), returning a CPU tensor containing the denoised movie.
